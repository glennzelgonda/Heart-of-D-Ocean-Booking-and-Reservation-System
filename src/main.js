  ...
  // CHECK AVAILABILITY FUNCTION (WITH LOADING EFFECT)
  function checkAvailability() {
    ...
    // Kunin ang button mismo. Kung wala, wag ituloy.
    const btn = document.querySelector('.check-availability-btn'); 
    if (!btn) {
    ...
    // 1. UI UPDATES (LOADING STATE)
    const originalText = btn.textContent; // I-save ang dating text
    btn.innerHTML = 'Checking... <span class="spinner-small">⏳</span>'; // Palitan ang text at icon
    btn.disabled = true; // Bawal i-click ulit
    btn.style.opacity = '0.7';
    btn.style.cursor = 'wait';

    // Alisin ang lumang messages
    const existingMessage = document.querySelector('.success-message, .error-message');
    if (existingMessage) existingMessage.remove();

    // 2. TIMEOUT (THE DELAY)
    // Gumamit tayo ng 3000ms (3 seconds) para sigurdong mapansin mo
    setTimeout(() => {
      console.log("Timeout finished, showing result."); // Debug log

      // Random result simulation
      const isAvailable = Math.random() > 0.3; 
      
      if (isAvailable) {
        showMessage('✅ This accommodation is available! You can proceed.', 'success');
      } else {
        showMessage('❌ Sorry, not available for selected dates.', 'error');
      }

      // 3. RESET BUTTON
      btn.innerHTML = originalText; // Ibalik ang dating text
      btn.disabled = false;
      btn.style.opacity = '1';
